...
const FormAddUser = React.memo(
  ({ props, appType, clickDashboardUser, submitForm, dataAddUser }) => {
    const Error = () => {
      if (CheckIsEmpty(dataAddUser.data)) {
        if (CheckIsEmpty(dataAddUser.data.error)) {
          switch (dataAddUser.data.error) {
            case "Duplicate email":
              return (
                <div className="alert alert-danger" role="alert">
                  {dataAddUser.data.error}
                </div>
              );
            ...
            default:
              return (
                <div className="alert alert-danger" role="alert">
                  Need a password error {dataAddUser.data.error[0]}
                </div>
              );
          }
        }
